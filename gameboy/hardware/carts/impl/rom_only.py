...
def check_header(cartridge: 'Cartridge'):
    ...
    checks.check_nintendo_logo(
        memory.read_many(
            NINTENDO_LOGO_BASE_ADDR, NINTENDO_LOGO.size,
        ),
    )
    checks.check_title(memory.read_many(TITLE_BASE_ADDR, 0x10))
    checks.check_manufacturer(memory.read_many(MANUFACTURER_CODE_ADDR, 4))
    checks.check_cgb_flag(memory.read_many(CGB_FLAG_ADDR, 1))
    checks.check_new_licensee_code(memory.read_many(NEW_LICENSE_CODE_ADDR, 2))
    checks.check_sgb_flag(memory.read_many(SGB_FLAG_ADDR, 1))
    checks.check_cartridge_type(memory.read_many(CARTRIDGE_TYPE_ADDR, 1))
    checks.check_rom_size(memory.read_many(ROM_SIZE_ADDR, 1))
    checks.check_ram_size(memory.read_many(RAM_SIZE_ADDR, 1))
    checks.check_destination_code(memory.read_many(DESTINATION_CODE_ADDR, 1))
    checks.check_old_licensee_code(memory.read_many(OLD_LICENSE_CODE_ADDR, 1))
    checks.check_version_number(memory.read_many(ROM_VERSION_ADDR, 1))
